...
pub async fn evaluate_canary_health(
    ...
    let phase_start: chrono::DateTime<chrono::Utc> = deploy_row.get("phase_start_time");
    let quality_baseline: Option<f64> = deploy_row.get("quality_score_baseline");
    let latency_baseline: Option<i64> = deploy_row.get("latency_ms_baseline");
    let requests_count: Option<i64> = deploy_row.get("requests_processed");

    let phase_duration = TimeDelta::hours(config.duration_hours as i64);
    let phase_elapsed = Utc::now() - phase_start;
    let phase_complete = phase_elapsed >= phase_duration;
    let requests_sufficient = requests_count.unwrap_or(0) >= config.min_requests as i64;
    ...
    let quality_delta = current_quality - (quality_baseline.unwrap_or(0.8) as f32);
    let latency_delta = (current_latency - latency_baseline.unwrap_or(400)) as i32;
    ...
    Ok(DeploymentMetrics {
        ...
        quality_score_baseline: quality_baseline.unwrap_or(0.8) as f32,
        quality_delta,
        latency_ms_current: current_latency as i32,
        latency_ms_baseline: latency_baseline.unwrap_or(400) as i32,
        latency_delta_ms: latency_delta,
        error_rate,
        requests_processed: requests_count.unwrap_or(0),
        is_healthy,
...
pub async fn check_and_execute_rollback(
    ...
    let current_quality_opt: Option<f64> = deploy_row.get("quality_score_current");
    let baseline_quality_opt: Option<f64> = deploy_row.get("quality_score_baseline");
    let current_latency: Option<i64> = deploy_row.get("latency_ms_current");
    let baseline_latency: Option<i64> = deploy_row.get("latency_ms_baseline");
    let error_rate_opt: Option<f64> = deploy_row.get("error_rate");

    let current_quality = current_quality_opt.unwrap_or(0.85) as f32;
    let baseline_quality = baseline_quality_opt.unwrap_or(0.85) as f32;
    let quality_delta = current_quality - baseline_quality;

    let current_latency = current_latency.unwrap_or(400);
    let baseline_latency = baseline_latency.unwrap_or(400);
    let latency_delta = (current_latency - baseline_latency) as i32;

    let error_rate = error_rate_opt.unwrap_or(0.0) as f32;
...
pub async fn advance_staged_deployment(
    ...
    let next_phase = match current_status.as_str() {
        "CANARY_IN_PROGRESS" => DeploymentPhase::Staged25Percent,
        "STAGED_25_PERCENT" => DeploymentPhase::Staged50Percent,
        "STAGED_50_PERCENT" => DeploymentPhase::FullDeployment,
...
pub async fn get_deployment_status(
    ...
    let status_str: String = dep_row.get("status");
    let deployment_percentage: Option<i64> = dep_row.get("deployment_percentage");
    let phase_start: chrono::DateTime<chrono::Utc> = dep_row.get("phase_start_time");
    let quality_current: Option<f64> = dep_row.get("quality_score_current");
    let quality_baseline: Option<f64> = dep_row.get("quality_score_baseline");
    let latency_current: Option<i64> = dep_row.get("latency_ms_current");
    let latency_baseline: Option<i64> = dep_row.get("latency_ms_baseline");
    let error_rate_val: Option<f64> = dep_row.get("error_rate");
    let req_count: Option<i64> = dep_row.get("requests_processed");
    let is_healthy: Option<bool> = dep_row.get("is_healthy");
    let rollback_trig: Option<bool> = dep_row.get("rollback_triggered");

    let current_phase = match status_str.as_str() {
        "CANARY_IN_PROGRESS" => DeploymentPhase::CanaryInProgress,
        "STAGED_25_PERCENT" => DeploymentPhase::Staged25Percent,
        "STAGED_50_PERCENT" => DeploymentPhase::Staged50Percent,
        "FULL_DEPLOYMENT" => DeploymentPhase::FullDeployment,
    ...
    Ok(DeploymentMetrics {
        ...
        agent_id,
        current_phase,
        deployment_percentage: deployment_percentage.unwrap_or(0) as i32,
        phase_start_time: phase_start,
        phase_duration_hours: 2, // TODO: track per phase
        quality_score_current: quality_current.unwrap_or(0.0) as f32,
        quality_score_baseline: quality_baseline.unwrap_or(0.0) as f32,
        quality_delta: (quality_current.unwrap_or(0.0)
            - quality_baseline.unwrap_or(0.0)) as f32,
        latency_ms_current: latency_current.unwrap_or(0) as i32,
        latency_ms_baseline: latency_baseline.unwrap_or(0) as i32,
        latency_delta_ms: (latency_current.unwrap_or(0)
            - latency_baseline.unwrap_or(0)) as i32,
        error_rate: error_rate_val.unwrap_or(0.0) as f32,
        requests_processed: req_count.unwrap_or(0),
        is_healthy: is_healthy.unwrap_or(false),
        rollback_triggered: rollback_trig.unwrap_or(false),
    })
}
